...
- (UIImage *)zf_cacheImageForRequest:(NSURLRequest *)request {
    if (request) {
        NSString *directoryPath = [NSString zf_cachePath];
        NSString *path = [NSString stringWithFormat:@"%@/%@", directoryPath, [request.URL.absoluteString md5String]];
...
- (NSUInteger)zf_failTimesForRequest:(NSURLRequest *)request {
    NSNumber *faileTimes = [self.zf_cacheFaileTimes objectForKey:[request.URL.absoluteString md5String]];
...
- (void)zf_cacheFailRequest:(NSURLRequest *)request {
    ...
    [self.zf_cacheFaileTimes setObject:@(times) forKey:[request.URL.absoluteString md5String]];
}
